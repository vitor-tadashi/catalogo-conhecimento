...
public class NegocioDAO extends GenericDAOImpl<NegocioBean, Integer> {
	...
	public NegocioBean obterNomeDesativado(NegocioBean negocioBean) throws SQLException, ClassNotFoundException {
		try {
			...
			TypedQuery<NegocioBean> query = entityManager.createQuery(
					"SELECT n FROM NegocioBean AS n WHERE n.areaAtuacao = :areaAtuacao AND ativo = 'n'",
					NegocioBean.class);
			NegocioBean negocioBeanDesativado = query.setParameter("areaAtuacao", negocioBean.getAreaAtuacao())
					.getSingleResult();
	...
	public List<NegocioBean> obterPorProjeto(ProjetoBean projeto) throws ClassNotFoundException, SQLException {
		...
		String sql = "  select" + "	n.areaAtuacao" + "  from" + "	Projeto as p inner join ProjetoNegocio as pn"
				+ "	on p.idProjeto = pn.idProjeto" + "	inner join Negocio as n" + "	on n.idNegocio = pn.idNegocio"
				+ "  where p.idProjeto = ?";
	...
	public List<NegocioBean> listarPorFuncionario(int id) throws ClassNotFoundException, SQLException {
		...
		String sql = "SELECT n.areaAtuacao, n.idNegocio FROM Funcionario AS f " + "INNER JOIN FuncionarioNegocio AS fn "
				+ "ON f.idFuncionario = fn.idFuncionario " + "INNER JOIN Negocio AS n "
				+ "ON n.idNegocio = fn.idNegocio " + "WHERE f.idFuncionario = ? AND f.ativo = ? ";
